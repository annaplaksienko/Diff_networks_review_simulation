#this is a modification of function from DiffNetFDR package
#allowing us to produce results for several alpha levels in one run

DiffNet.FDR_new <- function (X, group, alpha = 0.2, test.type = "pcor", parallel = F, 
                         nCpus = 4) 
{
    try(if (test.type %in% c("pcor", "pmat") == FALSE) 
        stop("The test type you provide is not include in this package."))
    X1 = X[group == unique(group)[1], ]
    X2 = X[group == unique(group)[2], ]
    if (test.type == "pcor") {
        out = DiffPCorr.test_new(X1, X2, alpha = alpha, parallel = parallel, 
                             nCpus = nCpus)
    }
    if (test.type == "pmat") {
        out = DiffPMat.test_new(X1, X2, alpha = alpha, parallel = parallel, 
                            nCpus = nCpus)
    }
    return(out)
}

DiffPCorr.test_new <- function (X, Y, alpha = 0.2, parallel = F, nCpus = 4) 
{
    n1 = dim(X)[1]
    n2 = dim(Y)[1]
    p = dim(X)[2]
    fit1 = lasso.estimator(X, lam1 = seq(40, 1)/20, parallel, 
                           nCpus)
    fit2 = lasso.estimator(Y, lam1 = seq(40, 1)/20, parallel, 
                           nCpus)
    ...
    for (k in seq(40)) {
        ...
        WW[, , k] = (T1 - T2)/sqrt((((1 - rho1^2)^2)/n1) + (((1 - 
                                                                  rho2^2)^2)/n2))
        diag(WW[, , k]) = 0
        WW[, , k] = WW[, , k] * upper.tri(WW[, , k])
        for (l in seq(1, 10)) {
            score[k] = score[k] + (sum(abs(WW[, , k]) > qnorm(1 - 
                                                                  l * (1 - pnorm(sqrt(log(p))))/10))/(l * (p^2 - 
                                                                                                               p) * (1 - pnorm(sqrt(log(p))))/10) - 1)^2
        }
    }
    k.min = which(score == min(score))[1]
    W.opt = WW[, , k.min]
    diag(W.opt) = 0
    M = 2000
    ...
    fdp = rep(NA, M)
    for (m in seq(M)) {
        fdp[m] = (2 - 2 * pnorm(tt[m])) * ((p^2 - p)/2)/max(sum(abs(W.opt) >= 
                                                                    tt[m]), 1)
    }
    result = list()
    for (j in 1:length(alpha)) {
        ...
        Diff.graph.connected = Diff.edge[Degree.Diff > 0, Degree.Diff > 
                                             0]
        Diff.net.connected = graph_from_adjacency_matrix(Diff.graph.connected, 
                                                         mode = "undirected", weighted = TRUE, diag = FALSE)
        Diff.net = graph_from_adjacency_matrix(Diff.edge, mode = "undirected", 
...
DiffPMat.test_new <- function (X, Y, alpha = 0.2, parallel = F, nCpus = 4) 
{
    n1 = dim(X)[1]
    n2 = dim(Y)[1]
    p = dim(X)[2]
    ...
    for (k in seq(40)) {
        ...
        Theta1 = (1 + diag(diag(fit1$r.hat[, , k])) %*% (fit1$beta[, 
                                                                   , k]^2) %*% D1)/(n1 * diag(fit1$r.hat[, , k]) %*% 
                                                                                        t(diag(fit1$r.hat[, , k])))
        Theta2 = (1 + diag(diag(fit2$r.hat[, , k])) %*% (fit2$beta[, 
                                                                   , k]^2) %*% D2)/(n2 * diag(fit2$r.hat[, , k]) %*% 
                                                                                        t(diag(fit2$r.hat[, , k])))
...
lasso.estimator <- function (X, lam1 = seq(40, 1)/20, parallel = F, nCpus = 4) 
{
    ...
    wrapper = function(i) {
        fit = glmnet(centered.X[, -i], centered.X[, i], lambda = lam1 * 
                         lam2[i])
        ...
        res[, i, ] = matrix(rep(centered.X[, i], length(lam1)), 
                            ncol = length(lam1)) - centered.X[, -i] %*% fit$beta
    ...
    for (k in seq(length(lam1))) {
        ...
        r.hat[, , k] = r.tilde[, , k] + diag(diag(r.tilde[, , 
                                                          k])) %*% beta[, , k] + t(beta[, , k]) %*% diag(diag(r.tilde[, 
                                                                                                                      , k]))
    }
